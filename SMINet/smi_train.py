...
from model.SMINet import SMINet
import pytorch_ssim
import pytorch_iou
...
args = {
    ...
    'tra_lbl_dir': '',
    'tra_edg_dir': '',            # path of training labels
    'image_ext': '.jpg',
    'label_ext': '.png',
...
def muti_loss_fusion(s_out,s0,s1,s2,labels_v):
...
def main():
    ...
    # ------- 3. define model --------
    # define the net
    net = nn.DataParallel(SMINet(in_channels=3))
    ...
    lr_schedule = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, gamma=0.1, step_size=)
    ...
    for epoch in range(args['epoch']):
        ...
        if (epoch+1) % 10 == 0:           # save model every 50 epochs
            torch.save(net.state_dict(), args['checkpoint'] + "SMINet_epoch_%d_trnloss_%3f_priloss_%3f.pth" % (
